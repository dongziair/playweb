...
MIN_TRADE_AMOUNT = 1000            # 单笔最小交易量，低于此值不下单
MAX_BALANCE_PROBE = 5_000_000      # 二分探测可交易额度的上限
MAX_CANDIDATES_TO_PROBE = 3        # 候选方向最大探测数量（当前未使用）
OPEN_SPREAD_THRESHOLD = 0.006       # 开仓最低收益率阈值，1%（扣费后 net_rate-1 须超过此值）
CLOSE_PROFIT_TARGET = 0.006         # 平仓目标收益率，1%（反向换回须 ≥ 开仓卖出量的 101%）
REBALANCE_INTERVAL = 0             # 仓位平衡执行间隔（秒），0 表示关闭自动仓位平衡
...
CLOSE_DIRECTION_LOG_TOP = 3        # 平仓方向日志最多显示前 N 个方向
CLOSE_RATE_PROBE_MARGIN = 0.004    # spot rate 距离方向最低目标在该范围内时，纳入详细平仓探测
...
def calc_net_rate(rates: Dict[str, float], sell: str, buy: str) -> Optional[float]:
    fee_mul = 1 - FEE
    direct = f"{sell}_{buy}"
    if direct in rates:
        return rates[direct] * fee_mul
    inverse = f"{buy}_{sell}"
    if inverse in rates and rates[inverse] != 0:
        return (1.0 / rates[inverse]) * fee_mul
    return None
...
class Bot:
    ...
    def __init__(self, dry_run=True):
        ...
        self.did_initial_rebalance = False
    ...
    def run(self):
        ...
        if REBALANCE_INTERVAL > 0:
            rebalance_desc = f"启动时执行 + 每 {REBALANCE_INTERVAL}s"
        else:
            rebalance_desc = "仅启动时执行一次"
        log.info(
            f"仓位平衡: {rebalance_desc} | "
    ...
    def _tick(self):
        ...
        if REBALANCE_INTERVAL > 0:
            should_rebalance = now - self.last_rebalance_time >= REBALANCE_INTERVAL
        elif not self.did_initial_rebalance:
            should_rebalance = True

        if should_rebalance:
            self.did_initial_rebalance = True
            self.last_rebalance_time = now
            if self._try_rebalance(summary):
                return

        # 平仓
        closed = self._check_and_close_position(summary, before_total, rates)
        ...
        if closed > 1:
            log.info(f"  本轮连续平仓 {closed} 笔")

        # 连续开仓，直到没有 >1% 的方向或余额不足。
        # 每次准备开新仓前，先再次检查是否已有仓位满足平仓条件，确保平仓优先。
        opened = 0
        while True:
            if self._check_and_close_position(summary, before_total, rates):
                closed += 1
                try:
                    time.sleep(0.3)
                    rates = self.pool_reader.get_rates()
            ...
            if not self._try_open_position(rates, summary):
                break
    ...
    def _check_and_close_position(self, summary: dict, before_total: int, rates: Dict[str, float]) -> bool:
        ...
        for idx, position in enumerate(self.positions):
            ...
            target_rate = target_amount_back / amount_out
            direction = (position["buy_resource"], position["sell_resource"])
            spot_rate = calc_net_rate(rates, position["buy_resource"], position["sell_resource"])

            entry = direction_stats.setdefault(direction, {
                "spot_rate": spot_rate,
                "min_target_rate": None,
                "best_rate_gap": None,
                "count": 0,
                "candidates": [],
            })
            ...
            if spot_rate is not None:
                rate_gap = spot_rate - target_rate
                if entry["min_target_rate"] is None or target_rate < entry["min_target_rate"]:
                    entry["min_target_rate"] = target_rate
                if entry["best_rate_gap"] is None or rate_gap > entry["best_rate_gap"]:
                    entry["best_rate_gap"] = rate_gap

            entry["candidates"].append({
                "index": idx,
                "position": position,
                "amount_out": amount_out,
                "target_amount_back": target_amount_back,
        ...
        ranked_directions = sorted(
            ...
            key=lambda item: (
                item[1]["best_rate_gap"] if item[1]["best_rate_gap"] is not None else float("-inf"),
                item[1]["count"],
            ),
            reverse=True,
        )
        direction_lines = []
        probe_directions = []
        for direction, info in ranked_directions:
            buy, sell = direction
            if info["spot_rate"] is None or info["min_target_rate"] is None:
                direction_lines.append(f"{buy}→{sell} x{info['count']} | 无可用汇率")
                continue
            delta_bp = (info["spot_rate"] - info["min_target_rate"]) * 10000
            direction_lines.append(
                f"{buy}→{sell} x{info['count']} | spot {info['spot_rate']:.4f} / 目标 {info['min_target_rate']:.4f} / 差值 {delta_bp:+.0f}bp"
            )
            if info["spot_rate"] + CLOSE_RATE_PROBE_MARGIN >= info["min_target_rate"]:
                probe_directions.append(direction)

        if direction_lines:
            log.info("  平仓方向: " + " || ".join(direction_lines[:CLOSE_DIRECTION_LOG_TOP]))

        if not probe_directions:
            probe_directions = [direction for direction, _ in ranked_directions[:2]]

        quote_candidates = []
        for direction in probe_directions:
            candidates = direction_stats[direction]["candidates"]
            candidates.sort(key=lambda item: item["target_rate"])
            quote_candidates.extend(candidates)
